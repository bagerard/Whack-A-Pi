...
def menu_screen(screen, scores):
	...
	for i in range(0,3):
		rect = left,top,width,height = (max_width * i) + 10, top, max_width - 20, max_height - 10
...
def game_screen(screen, elapsed, score, hiscore, wait = False):
	...
	if wait:
		...
		screen.blit(lab_wait,(align_h(lab_wait, 1, 0), 15 + lab_score_title.get_height()))
...
def _draw_win_screen(screen):
	...
	lab_btn=fnt_head.render("Submit", 1, YELLOW, )
	x = align_h(lab_btn, 2, 0)
	y = SIZE[1] - 30 - lab_btn.get_height()
	screen.blit(lab_btn, (x, y))
	btn = pygame.draw.rect(screen, YELLOW, (x-15, y -15, lab_btn.get_width() + 30, lab_btn.get_height() + 30), 1)

def win_screen(screen):
	line_pos = SIZE[1] / 3
	SPACE = 50
	inputboxes = []
	inputboxes.append(eztext.Input(maxlength=30, color=WHITE, focuscolor=YELLOW, prompt='First: ', x = 15, y=line_pos, hasfocus = True))
	line_pos += SPACE
	inputboxes.append(eztext.Input(maxlength=30, color=WHITE, focuscolor=YELLOW, prompt='Last: ', x = 15, y=line_pos))
	line_pos += SPACE
	inputboxes.append(eztext.Input(maxlength=15, color=WHITE, focuscolor=YELLOW, prompt='Twitter: @', x = 15, y=line_pos))
	line_pos += SPACE
	inputboxes.append(eztext.Input(maxlength=30, color=WHITE, focuscolor=YELLOW, prompt='Cont: ', x = 15, y=line_pos))
