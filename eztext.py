...
class Input:
	...
	def draw(self, surface):
		...
		field = self.font.size(self.prompt+"{:0>{fill}}".format(0, fill=self.maxlength))
		width = max(field[0], text.get_width())

		if self.hasfocus:
			boarder_c = self.focuscolor
		else:
			boarder_c = self.color
			
		boarder = pygame.draw.rect(surface, boarder_c, (self.x, self.y, width, text.get_height()), 1)
		self.top = boarder.top
		self.bottom = boarder.bottom
		self.left = boarder.left
		self.right = boarder.right
